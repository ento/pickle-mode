...
(defconst pickle-mode-font-lock-keywords-1
  (list
   ...
   '("\\(When\\|Then\\|Given\\|And\\|But\\)" . font-lock-keyword-face)
   '("\\(\\w+\\)(" 1 font-lock-function-name-face)
   '("Feature: \\(.*\\)" 1 font-lock-variable-name-face)
   '("Scenario: \\(.*\\)" 1 font-lock-preprocessor-face)
   '("\\(.*?\\): " . font-lock-type-face)
   '("\\(Background\\|Examples\\):" . font-lock-type-face)
...
(defun pickle-indent-line ()
  ...
  (if (bobp)
    ...
    (let ((not-indented t)
          (inhibit-message t)
          (keywords-to-indent "^[ \t]*\\(Feature\\|Background\\|Rule\\|Scenario\\|Scenario Outline\\|Scenario Template\\):")
          (next-keywords-to-indent '(("Rule" . "^[ \t]*\\(Feature\\):")
                                     ("Background" . "^[ \t]*\\(Feature\\|Rule\\):")
                                     ("Scenario" . "^[ \t]*\\(Feature\\|Rule\\):")
                                     ("Scenario Outline" . "^[ \t]*\\(Feature\\|Rule\\):")
                                     ("Scenario Template" . "^[ \t]*\\(Feature\\|Rule\\):")
                                   ))
          cur-indent)
      (if (looking-at "^[ \t]*Feature:")
          (setq cur-indent 0)
        (save-excursion
          (if (looking-at keywords-to-indent)
              (progn
                (setq keywords-to-indent (cdr (assoc (match-string 1) next-keywords-to-indent)))
                (if (not keywords-to-indent)
